...
Help(){
	...
	echo "[-w|--rscript|rscript] = path to the Rscript compiler of a R version 3.X.X (e.g. the one installed using R363.yml)"
...
# 	exit 1	
	...
	if [[ "$(cat $wd"input/"$to_py | wc -l)" -lt 3  ]]; then
	...
	cat $wd"input/"$to_py > ${tmp}/source_pmids.txt
	echo
	echo "create an efetch inputs directory"
	rm -rf "${tmp}/efetch_inputs"
	mkdir -p "${tmp}/efetch_inputs"
	cp -f $wd"input/"$to_py ${tmp}/efetch_inputs/.
	to_py=$(\ls ${tmp}/efetch_inputs/)
	#
	#D3NER_DIR="/home/musellla/D3NER-master"
	#
	...
	paste <(cat "${tmp}/efetch_inputs/${to_py}" | tr -d '\r') \
	...
	echo
	###
	while  [[ "$(cat ${tmp}/${tag}/status.log | wc -w)" -lt 1 ]];
		do			
			echo
			...
			if [[ ! -f ${tmp}/${tag}/data/${tag}_ids_to_genes.pbz2 ]]; then
		    	...
		    	#if python $wd"code/gene_ext_net_test.py" ${tmp}/${tag}/ $tag $ncores $it; then
		    		echo "Exit code of 0, success"
				else
		    		echo "Exit code of $?, failure"
		    		set -e
			...
			# echo 
			# echo "CHECKPOINT (1): ARE THERE BOTH MESH (>1) AND GENE (>2) TERMS IN THE MIXED NETWORK?"
			# genecount=$(csvcut -t -c 'category' ${tmp}/${tag}/data/${tag}_allxall_nodes_table_bh_to_cytoscape.tsv | grep 'GENE' | wc -l)
			# meshcount=$(csvcut -t -c 'category' ${tmp}/${tag}/data/${tag}_allxall_nodes_table_bh_to_cytoscape.tsv | grep 'MESH' | wc -l)
			# echo
			# if [[ "$genecount" -lt 2 ]] && [[ "$it" -lt 1 ]] || \
			# 	([[ "$meshcount" -lt 1 ]] && [[ "$it" -lt 1 ]]); then
			# 	echo "STOP: NOT ENOUGH GENEs OR MESHs TO PROCEED WITH MOTIFS EXTRACTION"
			# 	echo "Error: Significant co-occurrences only yielded a total of $genecount genes and $meshcount MESHs" > ${tmp}/${tag}/status.log
			# 	break
			# else
		  ...
		  #   		exit 1
			...
			if ! [ "$(cat ${tmp}/${tag}/status.log)" = "$x" ]; then
				...
				echo
				echo -e "Completed - A New Set of $(wc -l ${tmp}/efetch_inputs/${tag}_pmids_to_efetch.txt) PMIDs have been created"
				echo
				echo "##############################################################"	
				echo
				...
				#
				if [[ "$(cat ${tmp}/efetch_inputs/${to_py} | wc -l)" -lt 1 ]]; then
				...
				echo
				if [[ "$(cat ${tmp}/${tag}/${tag}_Genes_edges_table_subgraph.tsv | wc -l)" -lt 2 ]]; then 
	...
	if [[ "$(cat ${tmp}/${tag}/status.log)" = "$x" ]]; then
		...
		echo
		while [[ "$(ls  ${tmp}/${tag}/data/gene-subgraphs | wc -l)" -gt 2 ]]; # 2 because one edge table and one node table
			do 
				echo "Network-based Nodes Prioritization (SANTA)"
				...
				echo
				if [[ ! -f ${tmp}/${tag}/data/${tag}_ids_to_abs.pbz2 ]]; then			
					if python $wd"code/EDirect_pmid_to_abs_mesh.py" ${tmp}/${tag}/ $tag "${tmp}/efetch_inputs/${to_py}" $pw $ncores $etype; then
			    		echo "Exit code of 0, success"
					else
			    		echo "Exit code of $?, failure"
			    		set -e
  ...
  #   		# check line number of the motives list
   ...
   #  			tag=$prev				
		 ...
		 #    		exit 1
			...
			#[[ "$lines" -gt 1 ]] && echo "Completed - Motifs identified" || (echo -e "######## \n STOP: \n ${wd}data/${tag}_motifs_strengthonly.tsv is empty \n ########" && exit 1)
			echo				
		elif [[ "$(cat ${tmp}/${tag}/status.log)" = "Error: Significant co-occurrences only yielded a total of $genecount genes and $meshcount MESHs" ]]; then
			echo
	...
	sed -i "1s/.*/$(echo -e $new_header_node)/" */*Complete_nodes_table_subgraph.tsv
	new_header_node=$(echo $new_header_node | sed 's/\\tCommunityMembership//')
	sed -i "1s/.*/$(echo -e $new_header_node)/" */*Genes_nodes_table_subgraph.tsv	
	sed -i "1s/.*/$(echo -e $new_header_node)/" */*MeSH_nodes_table_subgraph.tsv	
	new_header_links="PMID\tKW1\tKW2\tlink\tlink_explicit\tQuerySet"
	sed -i "1s/.*/$(echo -e $new_header_links)/" */*_Complete_edges_literature_links.tsv
	echo
	echo
	echo "JOB FINISHED!" 
	echo	
